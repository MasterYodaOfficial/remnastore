...
Бот держит одно основное сообщение меню и редактирует его по inline-callback'ам.
Чат не должен засоряться навигационными сообщениями.

Если Telegram не дает отредактировать старое меню-сообщение, бот создает новое и обновляет ссылку на него в Redis.
...
5. если старое menu-message известно и доступно, бот обновляет caption или media
6. иначе бот отправляет новое photo-message и сохраняет `message_id` в Redis
