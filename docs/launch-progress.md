...
## Техническое примечание 2026-03-14

- в dev-режиме историческая цепочка Alembic схлопнута в одну инициализирующую миграцию `20260314_init_schema`
- после этого нужно пересоздать локальную БД или volume целиком; на старую dev-БД новая цепочка не накатывается
...
- [x] Реализовать admin flow обработки выводов
...
Дополнено 2026-03-13:
- backend добавил `GET /api/v1/admin/withdrawals` и `POST /api/v1/admin/withdrawals/{withdrawal_id}/status`
- админская обработка поддерживает переходы `new -> in_progress`, `new/in_progress -> paid`, `new/in_progress -> rejected`
- при отклонении создается compensating `ledger entry` типа `withdrawal_release`, а при выплате заявка закрывается без повторного денежного списания
- операции обработки вывода пишут `admin_action_log` с типом `withdrawal_status_change` и поддерживают `idempotency_key`

Дополнено 2026-03-14:
- закрыт реальный user flow вывода в web app: кнопка вывода больше не упирается в заглушку и работает через `POST /api/v1/withdrawals`
- пользователь вводит номер карты в отдельной форме заявки; перед отправкой номер проходит клиентскую и серверную валидацию по контрольной сумме
- пользовательский `GET /api/v1/withdrawals` теперь возвращает только маскированные реквизиты, без повторной выдачи полного номера карты в UI
- после отправки новая заявка сразу видна пользователю в блоке рефералки как история выводов со статусами `new / in_progress / paid / rejected / cancelled`
- при создании заявки пользователь получает уведомление, что заявка принята и передана администратору на рассмотрение
...
- [x] Добавить уведомления по заявкам на вывод
...
Дополнено 2026-03-13:
- notification service теперь отправляет события `withdrawal_paid` и `withdrawal_rejected` при ручной обработке заявок администратором
- интеграционные тесты `tests.test_admin_withdrawals` покрывают создание этих уведомлений вместе с admin processing flow
...
- [x] Добавить просмотр баланса и истории операций
...
- backend добавил `POST /api/v1/admin/accounts/{account_id}/status` для полной блокировки и снятия полной блокировки пользователя
- смена статуса требует обязательный комментарий, пишет `admin_action_log` и идемпотентна по `idempotency_key`
- полная блокировка теперь режет protected API, Telegram WebApp auth, новые topup/payment flows, wallet purchase подписки и новые заявки на вывод
- для Telegram добавлен internal-check по `telegram_id`: бот не отвечает на обычные сообщения и `/start`, если на аккаунте включена полная блокировка
- backend добавил `GET /api/v1/admin/withdrawals` и `POST /api/v1/admin/withdrawals/{withdrawal_id}/status`
- в `apps/admin` добавлен модуль `Выводы` с очередью pending заявок, карточкой обработки и действиями `взять в работу / отметить выплаченным / отклонить и вернуть резерв`
- backend добавил `GET /api/v1/admin/accounts/{account_id}/ledger-entries` с pagination и фильтром по `entry_type`
