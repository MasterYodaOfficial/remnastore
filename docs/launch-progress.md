...
- [ ] Убрать старые рублевые/копеечные названия вроде `referral_earnings_cents`
- [ ] Проверить, что во frontend баланс и реферальные суммы отображаются в рублях без плавающих дробей
- [ ] Проверить, что backend и frontend используют одну и ту же семантику денежных полей

### Account linking
- [ ] Прогнать `Browser -> Telegram`
- [ ] Прогнать `Telegram -> Browser`
- [ ] Проверить merge уже существующих аккаунтов
- [ ] Проверить повторное использование link token
- [ ] Проверить истечение link token

### Базовая надежность
- [ ] Добавить интеграционные тесты на account linking
...
- [ ] Просмотреть модели `accounts` и `schemas/account.py` на остатки старых денежных именований
- [ ] Определить целевую naming scheme для referral earnings и payout-остатков
- [ ] Подготовить первую миграцию на выравнивание именований
