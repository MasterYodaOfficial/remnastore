...
class AccountServiceFlowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_mark_telegram_account_blocked_updates_existing_account(self) -> None:
        account = await self._create_account(telegram_id=84)

        async with self._session_factory() as session:
            updated = await mark_telegram_account_blocked(session, telegram_id=84)
            self.assertIsNotNone(updated)
            assert updated is not None
            self.assertIsNotNone(updated.telegram_bot_blocked_at)

        async with self._session_factory() as session:
            self.assertIsNone(
                await mark_telegram_account_blocked(session, telegram_id=999999)
            )
